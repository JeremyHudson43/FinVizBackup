...
token = "aEFJcMihwLyFrGpeZXAB"
secret_key = "719829484614745"
RHI_rank = requests.get(
    "https://sentimentinvestor.com/api/v3/sort?limit=100&metric=RHI&token={0}&key={1}".format(token, secret_key)).json()
...
for stock in RHI_rank:
    yf_json = requests.get(
        "https://query2.finance.yahoo.com/v10/finance/quoteSummary/{}?modules=recommendationTrend".format(
            stock["ticker"])).json()
    stock_cap_json = requests.get(
        "https://query2.finance.yahoo.com/v10/finance/quoteSummary/{}?modules=defaultKeyStatistics".format(
            stock["ticker"])).json()
    stock_cap = 0
    if yf_json["quoteSummary"]["result"] != None:
        ...
        try:
            ...
            stock_cap = int(
                stock_cap_json["quoteSummary"]["result"][0]["defaultKeyStatistics"]["enterpriseValue"]["raw"])
